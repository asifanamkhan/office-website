    ...
    <div class="col-md-6">
        <div class="col-md-12">
            <div class="position-relative form-group">
                <label for="title" class=""><b>Title</b> </label>
                <input name="title" id="title" placeholder="Title" type="text" class="form-control
                   @if($error && array_key_exists('title',$error)) is-invalid @endif"
                       @if(isset($data))
                            value="{{ $data['title'] ?? '' }}"
                       @elseif(isset($slide))
                            value="{{ old('title', optional($slide['data'])->title) }}"
                       @endif>
        ...
        <div class="col-md-12">
            <div class="position-relative form-group">
                ...
                <textarea name="description" class="ckeditor form-control"
                        ...
                        {{ $data['description'] ?? '' }}
                    @elseif(isset($slide))
                        {{ old('description', optional($slide['data'])->description) }}
                    @endif
                </textarea>
            </div>
        </div>
    ...
    <div class="col-md-6">
                ...
                <select name="is_active" id="is_active"
                    ...
                    <option
                            @if(isset($data) && $data['is_active'] == 1)
                                selected
                            @elseif(!isset($data) && isset($slide) && $slide['data']['is_active'] == 1)
                                selected
                            @endif
                            value="1"> Active
                    </option>
                    <option
                            @if(isset($data) && $data['is_active'] == 0)
                                selected
                            @elseif(!isset($data) && isset($slide) && $slide['data']['is_active'] == 0)
                                selected
                            @endif
                            value="0">Deactivate
                    </option>
        ...
        <div class="col-md-12">
            <div class="position-relative row form-group">
                <label for="exampleFile" class="col-sm-3 col-form-label"><b>Slide image</b> <i class="fa fa-asterisk fa-xs is-invalid" aria-hidden="true"></i></label>
                <div class="col-md-9">
        ...
        <div class="col-md-12">
            ...
            @if ($slide && optional($slide['data'])->image)
                <div class="form-group" id="showImage">
                    <img src="{{ asset('images/slides/'.$slide['data']->image) }}" alt="" class="img-thumbnail" style="width: 550px; height: 350px">
                    <input type="hidden" value="{{ $slide['data']->image }}" name="oldImage">
                </div>
            @endif
        </div>
    </div>
