    ...
    <div class="app-page-title">
        <div class="page-title-wrapper">
            ...
            <div class="page-title-actions">
                <a class="btn-shadow mr-3 btn btn-shadow  btn-outline-primary" href="{{route('admin.settings.create')}}"> Add new setting</a>
            </div>
        </div>
    </div>
    <div class="main-card mb-3 card">
        <div class="card-body">
            ...
            <table style="width: 100%;" id="dataTable" class="table table-hover table-striped table-bordered">
                ...
                <tbody>
                @if($settings['status']==='success' && sizeof($settings['data']) )
                    @foreach ($settings['data'] as $setting)
                        <tr>
                            ...
                            <td>
                                <div class="badge {{$setting->is_active == 1 ? 'badge-success' : 'badge-danger'}} ">
                                    {{ $setting->is_active == 1 ? 'Active' : 'Deactivated' }}
                                </div>
                            </td>
                            <td>
                                <div class="btn-group">
                                    <a href="{{ route('admin.settings.show', $setting->id) }}" title="View">
                                    ...
                                    <form action="{{ route('admin.settings.destroy', $setting->id) }}" method="POST" class="delete_form">
                                        @csrf
                                        @method('DELETE')
