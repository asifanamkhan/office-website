        ...
        <div class="position-relative form-group">
            ...
            <input name="name" id="name" placeholder="Name" type="text" class="form-control
                    ...
                    @elseif(isset($category))
                        value="{{ old('name', optional($category['data'])->name) }}"
                    @endif>
            ...
            <select name="is_active" id="is_active"
                ...
                <option
                        @if(isset($data) && $data['is_active'] == 1)
                            selected
                        @elseif(!isset($data) && isset($category) && $category['data']['is_active'] == 1)
                            selected
                        @endif
                        value="1"> Active
                </option>
                <option
                        @if(isset($data) && $data['is_active'] == 0)
                            selected
                        @elseif(!isset($data) && isset($category) && $category['data']['is_active'] == 0)
                            selected
                        @endif
                        value="0">Deactivate
                </option>
        ...
        <div class="position-relative form-group">
            ...
            <textarea name="description" class="ckeditor form-control " id="description" rows="5" placeholder="Enter Description">
                @if(isset($data))
                    {{ $data['description'] ?? '' }}
                @elseif(isset($category))
                    {{ old('description', optional($category['data'])->description) }}
                @endif
            </textarea>
        </div>
    </div>
